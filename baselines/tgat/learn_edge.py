...
from gnn_tpp.tgat.utils import EarlyStopMonitor, RandEdgeSampler
...
MODEL_SAVE_PATH = f'gnn_tpp/tgat/saved_models/{args.prefix}-{args.agg_method}-{args.attn_mode}-{args.data}.pth'
...
def eval_one_epoch(hint, tgan, sampler, src, dst, ts, label):
    ...
    all_edge_scores, all_srcs, all_dsts = [], [], []
    with torch.no_grad():
        tgan = tgan.eval()
        TEST_BATCH_SIZE=30
        ...
        for k in range(num_test_batch):
            ...
            # save edge probs
            src_embed = tgan.tem_conv(src_l_cut, ts_l_cut, 1)
            dst_embed = tgan.tem_conv(dst_l_cut, ts_l_cut, 1)
            edge_scores = tgan.affinity_score(src_embed, dst_embed).squeeze(dim=-1)
            all_edge_scores.append(edge_scores)
            all_srcs.append(src_l_cut)
            all_dsts.append(dst_l_cut)
    ...
    print("Saving probs")
    with open("/efs/users/wngndr/tgat-edge-probs.p", "wb") as f:
        all_edge_scores = torch.cat(all_edge_scores).detach().cpu().numpy()
        all_srcs = [x for l in all_srcs for x in l]
        all_dsts = [x for l in all_dsts for x in l]
        pickle.dump((all_edge_scores, all_srcs, all_dsts, label), f)
...
for epoch in range(NUM_EPOCH):
    ...
    if not args.test:
        for k in range(num_batch):
            ...
            optimizer.zero_grad()
            tgan = tgan.train()
            #print(src_l_cut, dst_l_cut)
            #print(dst_l_fake, ts_l_cut)
            #input()
