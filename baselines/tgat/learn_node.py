...
from gnn_tpp.tgat.module import TGAN
from gnn_tpp.tgat.graph import NeighborFinder

class LR(torch.nn.Module):
    def __init__(self, dim, drop=0.3):
        super().__init__()
        self.fc_1 = torch.nn.Linear(dim, 1)
    ...
    def forward(self, x):
        ...
        #x = self.dropout(x)
        return self.fc_1(x).squeeze(dim=1)
...
parser.add_argument('--prefix', type=str, default='tpp')
...
USE_STATIC_CLF_TASK = True
PLOT_NODE_TRAJS = False

### set up logger
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger()
logger.setLevel(logging.DEBUG)
fh = logging.FileHandler('gnn_tpp/tgat/log/{}.log'.format(str(time.time())))
...
g_df = pd.read_csv('gnn_tpp/tgat/processed/ml_{}.csv'.format(DATA))
e_feat = np.load('gnn_tpp/tgat/processed/ml_{}.npy'.format(DATA))
n_feat = np.load('gnn_tpp/tgat/processed/ml_{}_node.npy'.format(DATA))

if DATA == "amazon":   # for amazon, we have pre-defined splits
    val_time, test_time = 18, 24
    src_types = e_feat[:,1]
else:
    val_time, test_time = list(np.quantile(g_df.ts, [0.6, 0.8]))
...
if USE_STATIC_CLF_TASK:
    # node is positive if any of its edges are positive
    node_labels = [0]*len(src_l)
    for user, label in zip(src_l, label_l):
        node_labels[user] = max(node_labels[user], label)

    label_l = []
    for user in src_l:
        label_l.append(node_labels[user])
...
#model_path = f'gnn_tpp/tgat/saved_models/{args.prefix}-{args.agg_method}-{args.attn_mode}-{DATA}.pth'
get_checkpoint_path = lambda epoch: f'gnn_tpp/tgat/saved_checkpoints/{args.prefix}-{args.agg_method}-{args.attn_mode}-{args.data}-{epoch}.pth'
model_path = get_checkpoint_path(2)
...
def eval_epoch(src_ld, dst_ld, ts_ld, label_ld, batch_size, lr_model, tgan, num_layer=NODE_LAYER):
    src_l, dst_l, ts_l, label_l = [], [], [], []
    seen_nodes = set()
    for src, dst, t, label in zip(src_ld, dst_ld, ts_ld, label_ld):
        if (src, t) not in seen_nodes:
            seen_nodes.add((src, t))
            src_l.append(src)
            dst_l.append(dst)
            ts_l.append(t)
            label_l.append(label)
    src_l, dst_l = np.array(src_l), np.array(dst_l)
    ts_l, label_l = np.array(ts_l), np.array(label_l)

    pred_prob, filt_labels = [], []
    loss = 0
    num_instance = len(src_l)
    num_batch = math.ceil(num_instance / batch_size)
    node_embs_over_time = defaultdict(list)
    with torch.no_grad():
        lr_model.eval()
        tgan.eval()
        for k in tqdm(range(num_batch)):
            ...
            if DATA == "amazon":   # only predict asins
                node_mask = np.array([src_types[src] == 0 for src in src_l_cut])
                src_l_cut, dst_l_cut = src_l_cut[node_mask], dst_l_cut[node_mask]
                ts_l_cut, label_l_cut = ts_l_cut[node_mask], label_l_cut[node_mask]
                filt_labels += list(label_l_cut)

            size = len(src_l_cut)
            src_embed = tgan.tem_conv(src_l_cut, ts_l_cut, num_layer)            
            for src, t, embed, label in zip(src_l_cut, ts_l_cut, src_embed, label_l_cut):
                embed = embed.detach().cpu().numpy()
                node_embs_over_time[src].append((t, embed, label))
            src_label = torch.from_numpy(label_l_cut).float().to(device)
            lr_prob = lr_model(src_embed).sigmoid()
            #print(lr_prob.shape, src_label.shape)
            loss += lr_criterion_eval(lr_prob, src_label).item()
            pred_prob += list(lr_prob.cpu().numpy())

    if PLOT_NODE_TRAJS:
        trajs, labels = [], []
        for traj in node_embs_over_time.values():
            ts, embs, ls = zip(*traj)
            labels.append(list(ls)[0])
            trajs.append(list(embs))
            
        pca = PCA(n_components=2).fit([x for l in trajs for x in l])
        for traj, label in zip(trajs, labels):
            if label == 0 and random.random() < 0.1:
                xs, ys = zip(*pca.transform(traj))
                xs, ys = list(xs), list(ys)
                plt.plot(list(xs), list(ys), color="blue", alpha=0.3)
        for traj, label in zip(trajs, labels):
            if label == 1 and random.random() < 0.1:
                xs, ys = zip(*pca.transform(traj))
                plt.plot(list(xs), list(ys), color="red", alpha=0.7,
                    marker=".")
                #plt.scatter(list(xs), list(ys), color="red")
        plt.savefig("/efs/users/wngndr/tgat-node-trajs.png")
        plt.close()

    #auc_roc = roc_auc_score(label_l, pred_prob)# if np.sum(label_l) > 0 else None
    auc_roc = roc_auc_score(filt_labels, pred_prob)# if np.sum(label_l) > 0 else None
